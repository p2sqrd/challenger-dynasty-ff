...
/**
 * The auction-budget table for one season: each manager's base, trade chips,
 * and current budget, folded from that season's budget_ledger (plus keeper
 * spend once the deadline locks it in). Rendered once per season tab.
 */
export async function BudgetSeasonTable({ season }: { season: BudgetSeason }) {
  const supabase = await createClient();

  // Keepers are still editable until the deadline, so their cost stays off the
  // public table until then. Once locked it folds into everyone's budget.
  ...
  // Only managers with a ledger entry this season are in the league.
              ...
              return (
                ...
                >
                  <td className="py-3 pl-4 pr-4">
                    <Link href={`/budget/${r.managerId}`} className="inline-flex">
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-6 text-center text-sm text-muted">
                  No budgets recorded for this season yet.
                </td>
              </tr>
            )}
          </tbody>
