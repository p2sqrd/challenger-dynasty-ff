...
import { loadDraft, resolveActingAs, toStateView } from "@/lib/rematch-load";
 ...
 *
 * `?actAs=<managerId>` is honoured on test drafts only; see the pick route.
 */
export async function GET(
  ...
  const actAs = new URL(request.url).searchParams.get("actAs");
  return NextResponse.json(
    toStateView(loaded, resolveActingAs(loaded, manager.id, actAs))
  );
}
